...
@section('mini title', 'Utilisateurs')
@section('adminContent')
<div class="w-full flex flex-col">

    <a href="{{route('users.create')}}" class="self-end">
        <button type="button" class="bg-cyan-600 hover:bg-cyan-500 cursor-pointer text-white font-bold py-2 px-4 rounded">Ajouter un Utilisateur</button>
    </a>
    ...
    <table class="w-full text-center mt-4 h-10 overflow-y-auto">
        ...
        <tbody class="bg-gray-100">
            ...
            <tr class="hover:bg-slate-200">
                ...
                <td class="py-4">{{ $user->prenom }}</td>
                <td class="py-4">{{ $user->adresse }}</td>
                <td class="py-4">{{ $user->ville }}</td>
                <td class="py-4 w-[50px] overflow-x-auto whitespace-nowrap">{{ $user->email }}</td>
                ...
                <td class="py-4 px-2">
                    <form action="{{route('users.destroy',$user)}}" method="post" class="flex gap-2 items-center justify-center">
                        @csrf
                        @method('DELETE')
