...
<div class="w-full flex flex-col">
    ...
    <form action="{{ route('unites.store') }}" method="post" enctype="multipart/form-data" class="w-full flex justify-end items-center">
        @csrf
        <label for="unite">unite :</label>
        <input type="text" name="unite" class="border rounded m-2">
    ...
    <table class="w-full text-center mt-4 h-10 overflow-y-auto">
        <thead>
            <tr>
                <th class="bg-gray-200 py-2 rounded-tl-2xl">unite</th>
        ...
        <tbody class="bg-gray-100">
            ...
            <tr class="hover:bg-slate-200">
                <td class="py-4 {{$loop->last?'rounded-bl-2xl':''}} ">{{ $unite->unite }}</td>
            ...
            @empty
            <tr>
                <td colspan='2' class='rounded-b-2xl p-2 font-bold'>pas des Unites</td>
            </tr>
            @endforelse
        </tbody>
    </table>
