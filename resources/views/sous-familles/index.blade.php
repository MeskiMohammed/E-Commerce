...
@section('title', 'Sous Famillies')
@section('mini title', 'Sous Famillies')
@section('adminContent')
<div class="w-full flex flex-col">

    <a href="{{route('sous-familles.create')}}" class="self-end">
        <button type="button" class="bg-cyan-600 hover:bg-cyan-500 cursor-pointer text-white font-bold py-2 px-4 rounded">Ajouter un sous Famille</button>
    </a>
    ...
    <table class="w-full text-center mt-4 h-10 overflow-y-auto">
        <thead>
            <tr>
                ...
                <th class="bg-gray-200 py-2">Famille</th>
        ...
        <tbody class="bg-gray-100">
            ...
            <tr class="hover:bg-slate-200">
                <td class="py-4 {{$loop->last?'rounded-bl-2xl':''}}">{{ $sousFamille->libelle }}</td>
                <td class="flex items-center justify-center h-full">
                    <img src="{{ asset($sousFamille->image) }}" alt="{{ $sousFamille->libelle }}" class="h-15 rounded-xl">
                </td>
                <td class="py-4">{{ $sousFamille->famille->libelle }}</td>
                <td class="py-4 px-2 {{$loop->last?'rounded-br-2xl':''}}">
                    <form action="{{route('sous-familles.destroy',$sousFamille)}}" method="post" class="flex gap-2 items-center justify-center">
                        @csrf
                        @method('DELETE')
                        <a href="{{route('sous-familles.edit',$sousFamille)}}">
            ...
            @empty
            <tr>
                <td colspan='4' class='rounded-b-2xl p-2 font-bold'>pas des sous Familles</td>
            </tr>
            @endforelse
    </table>
</div>
