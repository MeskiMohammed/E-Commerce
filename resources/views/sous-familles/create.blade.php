...
@section('title','Ajouter un sous Famille')
@section('mini title','Ajouter un sous Famille')
@section('adminContent')
<form action="{{ route('sous-familles.store') }}" method="post" enctype="multipart/form-data" class="w-full flex flex-col justify-center items-center">
    @csrf
    <table>
        <tr>
                ...
                <select name="famille_id" class="mx-2 w-[calc(100%-calc(0.2rem*5))] border border-1 rounded outline-none">
                    @php
                        ...
                        foreach($familles as $famille){
                            echo "<option value=$famille->id>$famille->libelle</option>";
                        }
