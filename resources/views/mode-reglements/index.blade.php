...
@section('title', 'Modes de Reglement')
@section('mini title', 'Modes de Reglement')
@section('adminContent')
<div class="w-full flex flex-col">

    <form action="{{ route('mode-reglements.store') }}" method="post" enctype="multipart/form-data" class="w-full flex justify-end items-center">
        @csrf
        <label for="mode_reglements">Mode de Reglement :</label>
        <input type="text" name="mode_reglements" class="border rounded m-2">
    ...
    <table class="w-full text-center mt-4 h-10 overflow-y-auto">
        <thead>
            <tr>
                <th class="bg-gray-200 py-2 rounded-tl-2xl">Mode de Reglement</th>
                <th class="bg-gray-200 py-2 rounded-tr-2xl">Action</th>
            </tr>
        </thead>
        <tbody class="bg-gray-100">
            @forelse($modeReglements as $mode)
            <tr class="hover:bg-slate-200">
                <td class="py-4 {{$loop->last?'rounded-bl-2xl':''}} ">{{ $mode->mode_reglements }}</td>
                <td class="py-4 w-min {{$loop->last?'rounded-br-2xl':''}}">
                    <form action="{{route('mode-reglements.destroy',$mode)}}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit">
            ...
            @empty
            <tr>
                <td colspan='2' class='rounded-b-2xl p-2 font-bold'>pas des Mode de Reglements</td>
            </tr>
            @endforelse
        </tbody>
    </table>
